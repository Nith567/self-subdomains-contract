...
# Deploy CryptoNomads Contract Script
...
# Deploy contract using forge script (not forge create)
print_info "Deploying CryptoNomads contract with placeholder scope..."

DEPLOY_CMD="forge script script/DeployProofOfHuman.s.sol:DeployCryptoNomads --rpc-url $RPC_URL --private-key $PRIVATE_KEY --broadcast"
...
# Extract deployed contract address from forge script output
# Look for the contract address in the broadcast files
BROADCAST_DIR="broadcast/DeployProofOfHuman.s.sol/$CHAIN_ID"
if [[ -f "$BROADCAST_DIR/run-latest.json" ]]; then
    CONTRACT_ADDRESS=$(jq -r '.transactions[] | select(.contractName == "CryptoNomads") | .contractAddress' "$BROADCAST_DIR/run-latest.json" | head -1)
else
    # Fallback: try to extract from console output
    # Deploy the contract
echo "Deploying CryptoNomads contract..."
CONTRACT_ADDRESS=$(forge script script/DeployCryptoNomads.s.sol:DeployCryptoNomads --rpc-url $RPC_URL --private-key $PRIVATE_KEY --broadcast --verify --etherscan-api-key $ETHERSCAN_API_KEY --force | grep "CryptoNomads deployed to:" | cut -d' ' -f4)
fi
...
if [ -n "$CELOSCAN_API_KEY" ]; then
    ...
    # verificationConfig struct: {olderThan: 18, forbiddenCountries: ["PRK"], ofacEnabled: false}
    ...
    forge verify-contract --constructor-args $CONSTRUCTOR_ARGS --chain-id $CHAIN_NAME $CONTRACT_ADDRESS src/cryptoNomads.sol:CryptoNomads --watch
...
echo
print_success "🎉 CryptoNomads Deployment Successful!"
echo
echo "Quick Links:"
...
echo "| Verification Config | olderThan: 18, forbiddenCountries: [NORTH_KOREA], ofacEnabled: false |"
if [ -n "$SCOPE_VALUE" ]; then
    echo "| Scope Value | $SCOPE_VALUE |"
fi
echo
print_success "✅ CryptoNomads Deployment Complete"
if [ -n "$SCOPE_SEED" ]; then
...
echo "1. Update your frontend .env with: NEXT_PUBLIC_SELF_ENDPOINT=\"$CONTRACT_ADDRESS\""
echo "2. Make sure NEXT_PUBLIC_SELF_SCOPE=\"crypto-nomads\" matches your SCOPE_SEED"
echo "3. Test the verification flow with your Discord bot!"
